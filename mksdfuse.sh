...
test -e $TARGET_DIR/emmc_boot.img || exit 0
...
TOTAL_SZ=`expr $ROOTFS_SZ + $BOOT_SIZE + $BOOT_SIZE + 2 + 80`
...
sudo mkfs.ext4 -F -m 0 -L rootfs /dev/mapper/${LOOP_DEV2}
test -d mnt || mkdir mnt

sudo mount /dev/mapper/${LOOP_DEV2} mnt
sync

echo $EMMC_ENV_OFFSET > env_offset

sudo cp $TARGET_DIR/rootfs.tar.gz mnt
sudo cp $TARGET_DIR/emmc_boot.img mnt
sudo cp $TARGET_DIR/params.bin mnt
sudo mv env_offset mnt
